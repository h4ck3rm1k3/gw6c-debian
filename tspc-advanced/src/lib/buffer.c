/*
-----------------------------------------------------------------------------
 $Id: buffer.c,v 1.11 2008/03/20 18:34:30 cnepveu Exp $
-----------------------------------------------------------------------------
...
void * buffer_append_space(Buffer *buffer, size_t len)
{
	...
	if (len > 0x100000) {
		Display(LOG_LEVEL_1, ELError, "buffer_append_space", HEX_STR_LEN_NOT_SUPPORTED, len);
		exit(-1);
	}
...
restart:
	...
	if (newlen > 0xa00000) {
		Display(LOG_LEVEL_1, ELError, "buffer_append_space", HEX_STR_ALLOC_NOT_SUPPORTED, newlen);
		exit(-1);
	}
