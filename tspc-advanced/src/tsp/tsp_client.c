/*
-----------------------------------------------------------------------------
 $Id: tsp_client.c,v 1.89.2.1 2009/03/13 01:16:32 carl Exp $
-----------------------------------------------------------------------------
...
gw6c_status         tspUpdateSourceAddr   ( tConf *Conf, pal_socket_t fd );
gw6c_status         tspTunnelNegotiation  ( pal_socket_t socket, tTunnel *t,
                                            tConf *conf, net_tools_t* nt,
                                            sint32_t version_index,
                                            tBrokerList **broker_list );
gw6c_status         tspSetupTunnel        ( tConf *, net_tools_t *,
                                            sint32_t version_index,
                                            tBrokerList **broker_list );
...
//
// -----------------------------------------------------------------------
gw6c_status tspUpdateSourceAddr( tConf *pConf, pal_socket_t fd )
{
...
// --------------------------------------------------------------------------
// tspTunnelNegotiation: Builds and sends a tunnel request to the Gateway6.
//   The Gateway6 will then offer a tunnel. The tunnel settings will be put
//   in the tTunnel structure(t).
//
gw6c_status tspTunnelNegotiation( pal_socket_t socket, tTunnel *tunnel_info, tConf *conf, net_tools_t* nt, sint32_t version_index, tBrokerList **broker_list )
{
  tPayload plin;
  tPayload plout;
  sint32_t tsp_status;
  ...
  // Send TSP tunnel request over to the Gateway6.
  ...
  // Process tunnel reply from Gateway6.
  ...
  // No need to acknowledge the tunnel offered by Gateway6.
  ...
  // Acknowledge TSP tunnel offer to Gateway6.
  memset(&plin, 0, sizeof(plin));
...
//
gw6c_status tspSetupTunnel(tConf *conf, net_tools_t* nt, sint32_t version_index, tBrokerList **broker_list)
{
  pal_socket_t socket;
  tCapability cap;
  ...
  // ----------------------------------------------------------------------
  {
    ...
    if( parse_addr_port( conf->server, &srvname, &conf->port_remote_v4, SERVER_PORT ) != 0 )
    {
      ...
      return make_status(CTX_NETWORKCONNECT, ERR_INVAL_GW6_ADDRESS);
    }
  ...
  if( conf->transport == NET_TOOLS_T_TCP || conf->transport == NET_TOOLS_T_TCP6 )
  {
    ...
    Display(LOG_LEVEL_2, ELInfo, "tspSetupTunnel", STR_GEN_CONNECTED_GATEWAY6, conf->server);
  }
  Display( LOG_LEVEL_3, ELInfo, "tspSetupTunnel", STR_GEN_USING_TSP_PROTO_VER, TSPProtoVerStr[version_index]);


  // --------------------------------------------
  // Get the TSP capabilities from the Gateway6.
  // --------------------------------------------
  Display(LOG_LEVEL_3, ELInfo, "tspSetupTunnel", STR_TSP_GETCAPABILITIES_FROM_GW6);
  ...
  // --------------------------------------------
  // Perform TSP authentication on the Gateway6.
  // --------------------------------------------
  ...
  // -------------------------------------------------------------------
  // Save the current Gateway6 address to the last-tsp-server.txt file.
  // -------------------------------------------------------------------
...
error_t RetrieveBrokerList( Gw6cBrokerList** ppBrokerList )
{
  ...
  {
    *ppBrokerList = pList = (Gw6cBrokerList*)pal_malloc( sizeof(Gw6cBrokerList) );
    ...
    while( tspBrokerList != NULL )
    {
      ...
      if( (tspBrokerList = tspBrokerList->next) != NULL )
        pList = (pList->next = (Gw6cBrokerList*)pal_malloc( sizeof(Gw6cBrokerList) ));
    }
  }
...
error_t RetrieveHAP6StatusInfo( HAP6StatusInfo** ppHAP6StatusInfo )
{
  ...
  hap6_status_info_copy = (HAP6StatusInfo *)pal_malloc(sizeof(HAP6StatusInfo));

  if (hap6_status_info_copy == NULL)
  {
    return GW6CM_UIS_ERRMEMORYSTARVATION;
  }
...
sint32_t tspMain(sint32_t argc, char *argv[])
{
  ...
  if( status_number(status) != SUCCESS )
  {
    gStatusInfo.nStatus = GW6CM_UIS_ERRCFGDATA;
    goto endtspc;
  }
  ...
  gszBrokerListFile = c.broker_list_file; // For BROKER_LIST gw6cmessaging message.
  ...
  if( (c.always_use_same_server == TRUE) && (pal_strlen(c.last_server_file) > 0) )
  {
    ...
    case TSP_REDIRECT_NO_LAST_SERVER:
      ...
      Display(LOG_LEVEL_2, ELInfo, "tspMain", HEX_STR_RDR_NO_LAST_SERVER_FOUND, c.last_server_file, original_server);
      break;

    case TSP_REDIRECT_CANT_OPEN_FILE:
      // Try the original server instead.
      trying_original_server = 1;
      Display(LOG_LEVEL_2, ELInfo, "tspMain", HEX_STR_RDR_CANT_OPEN_LAST_SERVER, c.last_server_file, original_server);
      break;

    default:
      Display(LOG_LEVEL_1, ELError, "tspMain", HEX_STR_RDR_ERROR_READING_LAST_SERVER, c.last_server_file);
      status = make_status(CTX_CFGVALIDATION, ERR_FAIL_LAST_SERVER);

      gStatusInfo.nStatus = GW6CM_UIS_ERRBROKERREDIRECTION;
      goto endtspc;
    }
  }
  else
  ...
  while( connected  &&  tspCheckForStopOrWait(0) == 0 )
  {
    ...
    case ERR_KEEPALIVE_TIMEOUT:
      // A keepalive timeout has occurred.
      {
        ...
        gStatusInfo.nStatus = GW6CM_UIS_ERRKEEPALIVETIMEOUT;
    ...
    case ERR_AUTHENTICATION_FAILURE:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRAUTHENTICATIONFAILURE;
    ...
    case ERR_NO_COMMON_AUTHENTICATION:
      // Configured authentication method is not supported by Gateway6.
      // User needs to change configuration. Abort.
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRNOCOMMONAUTHENTICATION;
    ...
    case ERR_INTERFACE_SETUP_FAILED:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRINTERFACESETUPFAILED;
    ...
    case ERR_TUN_LEASE_EXPIRED:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDIDLE;
        gStatusInfo.nStatus = GW6CM_UIS_ERRTUNLEASEEXPIRED;
    ...
    case ERR_INVAL_TSP_VERSION:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRTSPVERSIONERROR;
    ...
    case ERR_SOCKET_IO:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRSOCKETIO;
    ...
    case ERR_TSP_SERVER_TOO_BUSY:
      // The Gateway6 is currently too busy to process the TSP request.
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRTSPSERVERTOOBUSY;
    ...
    case ERR_TSP_GENERIC_ERROR:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRTSPGENERICERROR;
    ...
    case ERR_FAIL_SOCKET_CONNECT:
      // This means we could not connect to a Gateway6.
      ...
      // If that fails too, we'll go through the broker list (if any).
      {
        ...
        // Status update.
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRFAILSOCKETCONNECT;
        ...
        if( trying_original_server == 1 )
        {
          ...
          {
            Display (LOG_LEVEL_2, ELInfo, "tspMain", HEX_STR_RDR_READING_BROKER_LIST, c.broker_list_file);
            ...
            case TSP_REDIRECT_CANT_OPEN_FILE:
              ...
              Display (LOG_LEVEL_2, ELInfo, "tspMain", HEX_STR_RDR_CANT_OPEN_BROKER_LIST, c.broker_list_file);
              cycle++;
              tspFreeBrokerList(broker_list);
              broker_list = NULL;
              break;
            ...
            case TSP_REDIRECT_TOO_MANY_BROKERS:
              ...
              Display (LOG_LEVEL_1, ELError, "tspMain", HEX_STR_RDR_TOO_MANY_BROKERS, MAX_REDIRECT_BROKERS_IN_LIST);
              cycle++;
              tspFreeBrokerList(broker_list);
              broker_list = NULL;
              break;
            ...
            default:
              ...
              Display (LOG_LEVEL_1, ELError, "tspMain", HEX_STR_RDR_ERROR_READING_BROKER_LIST, c.broker_list_file);
              cycle++;
              tspFreeBrokerList(broker_list);
              broker_list = NULL;
              break;
    ...
    case EVNT_BROKER_REDIRECTION:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDIDLE;
        gStatusInfo.nStatus = GW6CM_UIS_EVNTBROKERREDIRECTION;
    ...
    case ERR_MEMORY_STARVATION:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRMEMORYSTARVATION;
    ...
    case ERR_TUNMODE_NOT_AVAILABLE:
      // Configured tunnel mode is not available on the Gateway6.
      // User needs to change configuration. Abort.
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRTUNMODENOTAVAILABLE;
    ...
    case ERR_TUNNEL_IO:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRTUNNELIO;
    ...
    case ERR_KEEPALIVE_ERROR:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRKEEPALIVEERROR;
    ...
    case ERR_BAD_TUNNEL_PARAM:
      // The tunnel information that the Gateway6 provided was bad.
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRBADTUNNELPARAM;
    ...
    case ERR_HAP6_SETUP:
      {
        ...
        gStatusInfo.nStatus = GW6CM_UIS_ERRHAP6SETUP;
    ...
    case ERR_HAP6_EXPOSE_DEVICES:
      // Home access error: Failed to make the devices available. Abort.
      {
        ...
        gStatusInfo.nStatus = GW6CM_UIS_ERRHAP6EXPOSEDEVICES;
    ...
    case ERR_INVAL_GW6_ADDRESS:
    case ERR_FAIL_RESOLV_ADDR:
      // Failed to parse gateway6 Address or resolve it.
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRINVALGW6ADDR;
    ...
    default:
      ...
      {
        gStatusInfo.eStatus = GW6C_CLISTAT__DISCONNECTEDERROR;
        gStatusInfo.nStatus = GW6CM_UIS_ERRUNKNOWN;
...
endtspc:
  ...
  {
    if( log_display_ok == 0 )
      DirectErrorMessage(STR_GEN_LAST_STATUS_CONTEXT, GW6CStatusContext[status_context(status)]);
    else
      Display(LOG_LEVEL_1, ELWarning, "tspMain", STR_GEN_LAST_STATUS_CONTEXT, GW6CStatusContext[status_context(status)]);
  }
