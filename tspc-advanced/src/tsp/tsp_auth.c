/*
-----------------------------------------------------------------------------
 $Id: tsp_auth.c,v 1.32 2008/03/13 19:52:33 cnepveu Exp $
-----------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// AuthANONYMOUS: Performs an anonymous authentication with the Gateway6.
//
gw6c_status AuthANONYMOUS(pal_socket_t socket, net_tools_t *nt, tConf *conf, tBrokerList **broker_list)
{
...
//   the Gateway6.
//
gw6c_status AuthPLAIN(pal_socket_t socket, net_tools_t *nt, tConf *conf, tBrokerList **broker_list)
{
  char BufferIn[1024];
...
// --------------------------------------------------------------------------
// AuthDIGEST_MD5: Performs a Digest-MD5 authentication with the Gateway6.
//
gw6c_status AuthDIGEST_MD5(pal_socket_t socket, net_tools_t *nt, tConf *conf, tBrokerList **broker_list, int version_index)
{
...
//
// Return values:
//   A gw6c_status status.
//
// --------------------------------------------------------------------------
gw6c_status tspAuthenticate(pal_socket_t socket, tCapability cap, net_tools_t *nt, tConf *conf, tBrokerList **broker_list, int version_index)
{
  gw6c_status status = make_status(CTX_TSPAUTHENTICATION, ERR_NO_COMMON_AUTHENTICATION);
