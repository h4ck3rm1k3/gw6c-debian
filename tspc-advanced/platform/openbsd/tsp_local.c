/*
-----------------------------------------------------------------------------
 $Id: tsp_local.c,v 1.21 2008/04/07 13:54:39 cnepveu Exp $
-----------------------------------------------------------------------------
...
char* tspGetLocalAddress(int socket, char *buffer, int size)
{
  ...
  if (getsockname(socket, (struct sockaddr *)&addr, &len) < 0) {
    Display(LOG_LEVEL_1, ELError, "TryServer", HEX_STR_ERR_FIND_SRC_IP);
    return NULL;
  }
...
//
gw6c_status tspStartLocal(int socket, tConf *c, tTunnel *t, net_tools_t *nt)
{
  ...
  if( getppid() != 1 )
  {
    Display(LOG_LEVEL_1, ELInfo, "tspStartLocal", HEX_STR_GOING_DAEMON);
    if (daemon(1, 0) == -1)
    {
      Display(LOG_LEVEL_1, ELError, "tspStartLocal", HEX_STR_CANT_FORK);
      return make_status(CTX_TUNINTERFACESETUP, ERR_INTERFACE_SETUP_FAILED);
    }
  }
