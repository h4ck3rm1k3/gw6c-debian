// **************************************************************************
// $Id: gw6cconfig.cc,v 1.23.2.1 2009/03/13 01:04:40 carl Exp $
//
...
bool GW6CConfig::ValidateConfigData( void )
{
  ...
  try
  {
    ...
    {
      m_lsValidationErrors.push_back( GW6C_UIS__G6C_PROXYANDKEEPALIVE );
      m_bValid = false;
    }
  }
  catch(...)
  ...
  try
  {
    ...
    if( v1 > v2 )
    {
      m_lsValidationErrors.push_back( GW6C_UIS__G6V_RETRYDELAYGREATERRETRYDELAYMAX );
      m_bValid = false;
    }
  }
  catch(...)
...
#ifdef HAP6
  // --------------------------------------------------------------
  // 7. If HAP6 Web is enabled, the document root cannot be empty.
  try
  {
  ...
  // --------------------------------------------------------------
  // 8. If HAP6 is built, the tunnel mode cannot be V4V6.
  try
  {
    Get_TunnelMode( sValue2 );
...
// --------------------------------------------------------------------------
void GW6CConfig::Get_RetryDelayMax( string& sRetryDelayMax ) const
{
  ASSERT_VALID_CONFIG;
...
void GW6CConfig::Set_RetryDelayMax( const string& sRetryDelayMax )
{
  ASSERT_VALID_CONFIG;
